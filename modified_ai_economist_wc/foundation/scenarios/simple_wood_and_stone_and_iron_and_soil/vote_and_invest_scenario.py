...
from modified_ai_economist_wc.foundation.base.base_env import BaseEnvironment, scenario_registry
from modified_ai_economist_wc.foundation.scenarios.utils import rewards, social_metrics
...
class UniformVoteAndInvest(BaseEnvironment):
    """
    World containing spatially-segregated wood and stone and iron and soil with stochastic regeneration.
    ...
    For controlling how resource regeneration behavior...
        ...
        Gradient Steepness: degree to which wood/stone/iron/soil are restricted to the top/bottom of the map.
    ...
    Args:
        ...
        starting_soil_coverage (int, float): Target coverage of soil at t=0.
        soil_regen_halfwidth (int): Regen halfwidth for soil.
        soil_regen_weight (float): Regen weight for soil.
        soil_max_health (int): Max soil units per soil source tile.
        soil_clumpiness (float): Degree of soil clumping.
    ...
    required_entities = ["Wood", "Stone", "Iron", "Soil"]
    ...
    def __init__(
        ...
        *base_env_args,
        ...
        starting_soil_coverage=0.1,
        soil_regen_halfwidth=0.5,
        soil_regen_weight=0.1,
        soil_max_health=1,
        soil_clumpiness=0.35,
        ...
        **base_env_kwargs
        ...
        if starting_soil_coverage >= 1:
            starting_soil_coverage /= np.prod(self.world_size)
        assert (starting_wood_coverage + starting_stone_coverage + starting_iron_coverage + starting_soil_coverage) < 0.75
        #
        ...
        self.layout_specs["Soil"]["starting_coverage"] = (
            float(starting_soil_coverage) * m
        )
        ...
        assert 0 < self.layout_specs["Soil"]["starting_coverage"] < 1
        #
        ...
        self.layout_specs["Soil"]["regen_halfwidth"] = int(soil_regen_halfwidth)
        ...
        assert 0 <= self.layout_specs["Soil"]["regen_halfwidth"] <= 3
        #
        ...
        self.layout_specs["Soil"]["regen_weight"] = float(soil_regen_weight)
        ...
        assert 0 <= self.layout_specs["Soil"]["regen_weight"] <= 1
        #
        ...
        self.layout_specs["Soil"]["max_health"] = int(soil_max_health)
    ...
    def make_source_prob_maps(self):
        ...
        return {
            ...
            "Soil": prob_gradient[-1::-1] * self.layout_specs["Soil"]["starting_coverage"],
        }
    ...
    def reset_agent_states(self):
        ...
        communication_resource_words = ['a', 'b', 'c', 'd']

        if self.n_agents == 6:
            self.world.agents[0].state["endogenous"]["Communication"] = ['a', 'b', 'c', 'd']
            self.world.agents[1].state["endogenous"]["Communication"] = ['d', 'a', 'b', 'c']
            self.world.agents[2].state["endogenous"]["Communication"] = ['c', 'd', 'a', 'b']
            self.world.agents[3].state["endogenous"]["Communication"] = ['b', 'c', 'd', 'a']
            self.world.agents[4].state["endogenous"]["Communication"] = ['d', 'c', 'b', 'a']
            self.world.agents[5].state["endogenous"]["Communication"] = ['b', 'a', 'd', 'c']
        
        else:
            for agent in self.world.agents:
                index = np.random.permutation(4) 
                agent.state["endogenous"]["Communication"] = [communication_resource_words[index[0]], communication_resource_words[index[1]], communication_resource_words[index[2]], communication_resource_words[index[3]]]
        ...
        for agent in self.world.agents:
            ...
            agent.state["inventory"]["Soil"] = int(self.starting_agent_coin)
            agent.state["escrow"]["Soil"] = 0
            
            agent.state["endogenous"]["Labor"] = 0.0 
            agent.state["endogenous"]["VoteInvest"] = float(self.starting_agent_coin) * np.random.random(4)
        ...
        self.world.planner.state["endogenous"]["VoteInvest"] = float(self.starting_agent_coin) * np.random.random(4)
        self.world.planner.state["endogenous"]["Communication"] = []
